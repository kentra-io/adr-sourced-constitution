...
- **CLI:** `adr new --rule <text>` composes the section (canonical position: last body section). A `--body-file` MAY instead carry its own `## Rule` section; supplying both is an error; an empty/whitespace-only Rule section is invalid. `supersede` gains the same `--rule`/body-file semantics for the superseding ADR. Rule text is plain prose: lines beginning with `#` are invalid, duplicate `## Rule` sections are invalid, and `## Rule` may not be the first heading of a founding file (reserved word) — malformed rule input is rejected on every path (flag, body-file, founding-file), never silently swallowed.
...
### 2.14 v0.3 revision block — founding file is an ADR body, config command group (issues #18–#20)

v0.2 (§2.13/§7 of the spec) still supplied a founding project's principles through a **multi-principle `--founding-file` grammar**: one principle per `## <title>` heading, each optionally followed by its own attached `## Rules` block, with `init` seeding one ADR per principle. That grammar shipped, then proved to be its own kind of drift risk — a second, founding-only body grammar next to `adr new --body-file`'s, and no machine-readable way for a skill to learn the live `constitution.yml` vocabulary short of hardcoding it (the root cause of issue #17's `github` vs `github-issue` drift) or hand-serializing YAML to change one field. Six milestones, tracked in commit order:

| Decision | One-liner | Commit |
|---|---|---|
| **`config schema`** | `constitution config schema` prints `constitution.yml`'s whole vocabulary as JSON, derived from the real validator — a skill introspects the live schema instead of hardcoding it. | M1, `2ccacb6` |
| **`config set`** | `constitution config set <key> <value>` writes one dotted key, re-validates the whole resulting config in memory, and writes atomically only if that passes — no skill ever hand-serializes `constitution.yml`. Settable: `consent.policy`, `sourceTracking.type`, `sourceTracking.pattern`, `agentInstructions.targets`, `skills.trees`. Refused with a redirect: `phase` → `constitution seal`, `categories` → `adr new --new-category`; `schemaVersion` is not settable at all (no migration machinery). | M2, `e082e56` |
| **`init --source-tracking`/`--source-pattern`** | Source tracking is now configurable at `init` time, not only via a later config edit. | M3, `5641ff7` |
| **Founding file is an ordinary ADR body** | `--founding-file` now takes exactly what `adr new --body-file` takes: a single MADR body (mandatory `## Context and Problem Statement`, `## Considered Options`, `## Decision Outcome`; optional `## Decision Drivers`; optional `## Rules` in the `### <category>`/`#### <slug>` grammar). `init` seeds exactly **one** founding ADR — always `ADR-0001`, titled "Founding constitution" — instead of one per principle. Any other, non-mandatory section (e.g. a prose "## Deferred bets") is preserved verbatim in the log but never rendered. `parseFoundingFile`, the `principle` type, `gatherPrinciples`, `foundingBody`, and `foundingLabel` were deleted along with the multi-principle grammar they implemented. | M4, `d6e40cd` |
| **Realistic full-project founding fixture** | A judged-by-reading golden exercising the new single-body grammar end to end, replacing goldens written against the deleted grammar. | M5, `6cabe77` |
| **`constitution-init` rewritten onto Found/Validate/Settle + the new grammar** | The Layer-2 skill's founding-file authoring section now teaches the single-ADR-body grammar (never the deleted multi-principle one) and points at `constitution config schema` for the live vocabulary instead of restating it. | M6, `4181069` |

Every shipped doc that taught the deleted multi-principle grammar or instructed a hand-edit of `constitution.yml` was swept and rewritten in the same change (this milestone) — `README.md`, `adr-sourced-constitution.md` §7, `skills/adr-draft/SKILL.md`, `skills/constitution-gov/SKILL.md` — with the plugin version bumped past `0.2.1` so `claude plugin update` (which diffs `plugin.json`'s version field, not content) actually redistributes the rewritten skills.
